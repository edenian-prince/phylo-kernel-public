...
mkdir ../data/beast/
cp ../data/rep-1-seq-density-0.5-biased-metadata.csv ../data/beast
cp ../data/rep-1-seq-density-0.5-biased-alignment.fasta ../data/beast/
cd ../data/beast/

#make new strain name col with format name_hct_date
awk -F"," 'OFS="\t" {$1=$2"_"$3"_"$4; print}' rep-1-seq-density-0.5-biased-metadata.csv > meta.tsv
...
cat rep-1-seq-density-0.5-biased-alignment.fasta | seqkit replace --ignore-case --kv-file "kv.txt" --pattern "^(\w+)" --replacement "{kv}" > beast_rep-1-seq-density-0.5-biased-alignment.fasta
